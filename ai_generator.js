    ...
    quadratic: {
      graphs: {
        ...
        basic: function() {
          ...
          const b = -2 * h, c = h * h + k;
          const eqStr = `x^2${b !== 0 ? (b > 0 ? ` + ${b}x` : ` - ${Math.abs(b)}x`) : ''}${c !== 0 ? (c > 0 ? ` + ${c}` : ` - ${Math.abs(c)}`) : ''}`;
          return {
            ...
            solutionHtml: `<p>平方完成を行います。</p><p>$y = x^2 ${b !== 0 ? (b > 0 ? `+ ${b}x ` : `- ${Math.abs(b)}x `) : ''}${c !== 0 ? (c > 0 ? `+ ${c}` : `- ${Math.abs(c)}`) : ''}$</p><p>$y = (x ${h >= 0 ? `- ${h}` : `+ ${Math.abs(h)}`})^2 ${k >= 0 ? `+ ${k}` : `- ${Math.abs(k)}`}$</p><p>よって頂点は $(${h}, ${k})$ です。</p>`,
        ...
        advanced: function() {
          ...
          const eqStr = `${a !== 1 ? a : ''}x^2${B !== 0 ? (B > 0 ? ` + ${B}x` : ` - ${Math.abs(B)}x`) : ''}${C !== 0 ? (C > 0 ? ` + ${C}` : ` - ${Math.abs(C)}`) : ''}`;
          return {
      ...
      equations: {
        ...
        standard: function() {
          ...
          const bStr = bv !== 0 ? (bv > 0 ? ` + ${bv}x` : ` - ${Math.abs(bv)}x`) : '';
